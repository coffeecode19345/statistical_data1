        ...
        for item in category_data:
            ...
            if images:
                ...
                with st.container():
                    ...
                    # Thumbnails
                    with st.container(key=f"thumb_container_{item['folder']}"):
                        cols = st.columns(min(len(images), 5))  # Limit to 5 thumbnails per row
                        for idx, (thumb_name, thumb_image, _, _, _) in enumerate(images):
                            with cols[idx % 5]:
                                if st.button("", 
                                    ...
                                    st.rerun()
                                st.image(thumb_image, width=100, caption=None)
